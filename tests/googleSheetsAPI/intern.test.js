...
const { getInterns, getInternById, writeInternChoices } = require('../../src/service/intern');
...
test('Should fetch first intern', async () => {
    const intern = await getInternById(0);
...
test('Should write choices to first intern', async () => {
    ...
    var intern = (await getInterns())[0];
    const past_choices = intern.choices;
    const new_choices = ['1','2','3','4'];
    const choices = await writeInternChoices(0, new_choices);
    expect(choices).toEqual(new_choices);
    await writeInternChoices(0, past_choices);
    intern = (await getInterns())[0];
